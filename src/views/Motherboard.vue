<template>
  <div class="container">
    ...
    <div class="row mt-2" v-for="(note, index) in mother" :key="note.id">
...
export default {
  ...
  methods: {
    async updateData() {
      try {
        let res = await this.$http.get("http://localhost:3000/mother");
        this.mother = await res.json();
        this.errMsg = "";
      } catch (e) {
        console.error(e);
      }
    ...
    async onSave() {
      ...
      let checkPhone = this.mother.filter(i => i.phone == note.phone);
      if (checkPhone != false) {
        this.errMsg = "Такой номер уже существует";
        return;
      }
      try {
        await this.$http.post("http://localhost:3000/mother", note);
        this.updateData();
      } catch (e) {
        console.error(e);
      }
    ...
    async onDelete(id) {
      try {
        await this.$http.delete("http://localhost:3000/mother/" + id);
        this.updateData();
      } catch (e) {
        console.log(e);
      }
    ...
    EditModOn(index) {
      this.name = this.mother[index].name;
      this.articule = this.mother[index].articule;
      this.phone = this.mother[index].phone;
      this.id = this.mother[index].id;
      this.isEdit = true;
    },
    EditModOff() {
      this.name = "";
    ...
    async onSaveEdit(id) {
      ...
      try {
        await this.$http.put("http://localhost:3000/mother/" + id, note);
        this.updateData();
      } catch (e) {
        console.log(e);
      }
